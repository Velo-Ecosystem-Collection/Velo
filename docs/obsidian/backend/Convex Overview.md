...
- `gas`: Testnet-only Gas policy, relayer metadata, safe projections, console/API-key authorization, policy evaluation, public `api.gas.public_api.sponsor`/`submit` Node actions, and internal atomic admission/submit lifecycle boundaries. Public actions enforce payload bounds and typed dependency outcomes; the two Next.js Gas routes are bounded HTTP callers of these seams.
...
Authenticated Convex functions call project/organization helpers. Gas console functions use the shared project role guard (`viewer` reads, `editor` policy updates, `owner` relayer updates) and return field-by-field projections. Public merchant actions use hashed API-key lookup and project payment-access checks. Gas sponsor/submit actions use their independent API-key query, enforce action-level bounds, and do not consult `paymentAccessActive`; dependency failures are classified separately from invalid input and invariant failures. Internal workers use `internal` references and must still validate ownership/state when crossing boundaries.
