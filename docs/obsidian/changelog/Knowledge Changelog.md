...
- Completed Gas Station Sub-sprint 3.3: added the Node-runtime `api.gas.public_api.sponsor` Convex orchestration action and internal API-key authorization query. The action authorizes before parsing, trims and hashes the idempotency/XDR inputs without persisting raw values, derives signed Testnet facts including the optional Convex-safe inner `maxTime`, delegates exactly once to atomic admission, and returns validated safe success/rejection/error outcomes. Focused action tests cover payment-access independence, one reservation, policy denial without accounting consumption, replay/conflict/duplicate handling, typed malformed/network/transaction/key failures, and redaction. The Next.js HTTP route, relayer execution, submission, cleanup, SDK, and UI remain planned.
- Completed Gas Station Sub-sprint 3.2: added private `internal.gas.admission.reserve` with server-boundary API-key/project revalidation, idempotency-first replay/conflict handling, project-scoped reserved transaction uniqueness, UTC daily rollover, Gas wallet-hour quota buckets backed by `rateLimitBuckets`, exact `innerMaxFeeStroops + 100` reservation accounting, earlier-of-`maxTime`/15-minute expiry, redacted replayable policy-denial logs, 30-day retention, and atomic policy/bucket/log writes. Deterministic Convex tests cover accounting, replay/conflict, duplicate hashes, denial/retry behavior, rollovers, revocation/scope mismatch, and concurrent cap/quota boundaries. Public sponsor orchestration, routes, submission, cleanup, and relaying remain out of scope.
- Completed Gas Station Sub-sprint 3.1: added the reusable `@repo/stellar` Testnet transaction-envelope helper and the `gas/envelope.ts` adapter. Signed non-FeeBump envelopes are structurally restricted to one contract invocation, source signatures are verified against the Testnet signature base with Public-network mismatch detection, and source wallet, Testnet hash, inner maximum fee, and contract target are derived from XDR. Typed `invalid_request`, `invalid_signature`, `wrong_network`, and `unsupported_transaction` failures contain no raw XDR or parser details. Deterministic Gas fixtures and constructed non-contract-host/invalid-target cases are covered; public APIs, submission, and relaying remain unimplemented.
- Completed Gas Station Sub-sprint 2.5: added authenticated `gas/queries.listLogsPage` with viewer-or-higher project authorization, unchanged Convex pagination options, newest-first `by_project_id_and_created_at` access, preserved cursor metadata, and field-by-field `GasLogProjection` redaction. Optional stored values return as `null` and stroop amounts as decimal strings; Convex IDs, project/internal correlation fields, retention metadata, raw XDR, signatures, credentials, and custody material remain outside the result. Integration tests prove pagination completeness, interleaved-project isolation, authorization failures, exact response shapes, and extra-argument rejection. Status, wallet, and date filters remain later work.
